...
class SolarParser:
    ...
    def _try_ascii_schema(blocks: Dict[str, bytes]) -> Dict[str, object]:
        ...
        SolarParser._decode_this_inverter(parsed, state)
        SolarParser._apply_energy_dashboard_calculations(state)

        return state

    @staticmethod
    def _decode_this_inverter(
        parsed: Dict[str, Tuple[str, List[str]]],
        state: Dict[str, object],
    ) -> None:
        """Decode blocks for the newer Siseli inverter (GPeV/PWDK/5yXy family)."""
        _SENTINEL = 32767

        # GPeV: grid voltage and frequency
        vals = parsed.get("GPeV", ("", []))[1]
        if len(vals) >= 1:
            v = SolarParser._to_float(vals[0])
            if v is not None:
                state["grid_v"] = round(v, 1)
        if len(vals) >= 4:
            hz = SolarParser._to_float(vals[3])
            if hz is not None:
                state["grid_hz"] = round(hz, 1)

        # yqs1: grid V/Hz fallback, pv_w, inverter lifetime hours
        vals = parsed.get("yqs1", ("", []))[1]
        if len(vals) >= 1 and "grid_v" not in state:
            v = SolarParser._to_float(vals[0])
            if v is not None:
                state["grid_v"] = round(v, 1)
        if len(vals) >= 2 and "grid_hz" not in state:
            hz = SolarParser._to_float(vals[1])
            if hz is not None:
                state["grid_hz"] = round(hz, 1)
        if len(vals) >= 5:
            pw = SolarParser._to_int(vals[4])
            if pw is not None and pw != _SENTINEL:
                state.setdefault("pv_w", pw)
                state.setdefault("generation_power_w", pw)
                state.setdefault("c_generation_power_w", SolarParser._scale_main_power(pw))
        if len(vals) >= 15:
            hrs = SolarParser._to_int(vals[14])
            if hrs is not None and hrs != _SENTINEL:
                state["inverter_lifetime_hours"] = hrs

        # RrrX: output voltage, frequency, current
        vals = parsed.get("RrrX", ("", []))[1]
        if len(vals) >= 1:
            v = SolarParser._to_float(vals[0])
            if v is not None and v > 50:
                state.setdefault("out_v", round(v, 1))
        if len(vals) >= 2:
            hz = SolarParser._to_float(vals[1])
            if hz is not None:
                state.setdefault("out_hz", round(hz, 1))
        if len(vals) >= 3:
            ia = SolarParser._to_float(vals[2])
            if ia is not None:
                state["out_current_a"] = round(ia, 2)

        # 5yXy: output V/Hz/VA/W/%, reactive power, uptime
        vals = parsed.get("5yXy", ("", []))[1]
        if len(vals) >= 1:
            v = SolarParser._to_float(vals[0])
            if v is not None and v > 50:
                state["out_v"] = round(v, 1)
        if len(vals) >= 2:
            hz = SolarParser._to_float(vals[1])
            if hz is not None:
        ...
        if len(vals) >= 13:
            ...
            if upt is not None:
                state["uptime_min"] = upt

        # PWDK: battery voltage, charge current, series count, thresholds
        vals = parsed.get("PWDK", ("", []))[1]
        if len(vals) >= 1:
            bv = SolarParser._to_float(vals[0])
            if bv is not None:
                state["bat_v"] = round(bv, 2)
        if len(vals) >= 2:
            bc = SolarParser._to_float(vals[1])
            if bc is not None:
                state["bat_charge_current"] = round(bc, 2)
        if len(vals) >= 3:
            sc = SolarParser._to_int(vals[2])
            if sc is not None:
                state["bat_series_count"] = sc
        if len(vals) >= 4:
            sv = SolarParser._to_float(vals[3])
            if sv is not None:
                state["bat_shutdown_v"] = round(sv, 1)
        if len(vals) >= 5:
            lv = SolarParser._to_float(vals[4])
            if lv is not None:
                state["bat_low_alarm_v"] = round(lv, 1)

        # 41Q7: battery capacity / SOC
        vals = parsed.get("41Q7", ("", []))[1]
        if len(vals) >= 1:
            cap = SolarParser._to_int(vals[0])
            if cap is not None and 0 <= cap <= 100:
                state["bat_cap"] = cap
                state["bms_current_soc"] = cap

        # ucsX: discharge current, charging voltages, max charge, battery Ah, priority, BMS flag
        vals = parsed.get("ucsX", ("", []))[1]
        if len(vals) >= 6:
            dc = SolarParser._to_float(vals[5])
            if dc is not None:
                state["dischg_current"] = round(dc, 2)
        if len(vals) >= 7:
            bv = SolarParser._to_float(vals[6])
            if bv is not None:
                state["strong_charging_voltage_v"] = round(bv, 1)
                state["bulk_v"] = round(bv, 1)
        if len(vals) >= 8:
            fv = SolarParser._to_float(vals[7])
            if fv is not None:
                state["float_charging_voltage_v"] = round(fv, 1)
                state["float_v"] = round(fv, 1)
        if len(vals) >= 9:
            ev = SolarParser._to_float(vals[8])
            if ev is not None:
                state["battery_equalization_voltage_v"] = round(ev, 1)
        if len(vals) >= 10:
            mc = SolarParser._to_float(vals[9])
            if mc is not None:
                state["maximum_total_charging_current_a"] = round(mc, 1)
                state["max_chg"] = round(mc, 1)
        if len(vals) >= 11:
            ah = SolarParser._to_int(vals[10])
            if ah is not None:
                state["bat_nominal_ah"] = ah
        if len(vals) >= 16:
            pri = SolarParser._to_int(vals[15])
            if pri is not None:
                state["charging_priority_order"] = pri
        if len(vals) >= 23:
            bms = SolarParser._to_int(vals[22])
            if bms is not None:
                state["bms_communication_normal"] = "Yes" if bms == 1 else "No"

        # O0MF: ambient temperature, charge/discharge current fallbacks
        vals = parsed.get("O0MF", ("", []))[1]
        if len(vals) >= 1:
            at = SolarParser._to_float(vals[0])
            if at is not None:
                state["ambient_temp_c"] = round(at, 1)
        if len(vals) >= 3:
            bc = SolarParser._to_float(vals[2])
            if bc is not None and "bat_charge_current" not in state:
                state["bat_charge_current"] = round(bc, 2)
        if len(vals) >= 4:
            dc = SolarParser._to_float(vals[3])
            if dc is not None and "dischg_current" not in state:
                state["dischg_current"] = round(dc, 2)

        # oh1V: PV voltages
        vals = parsed.get("oh1V", ("", []))[1]
        if len(vals) >= 1:
            pv = SolarParser._to_float(vals[0])
            if pv is not None:
                state["pv_v"] = round(pv, 1)
        if len(vals) >= 2:
            pv1 = SolarParser._to_float(vals[1])
            if pv1 is not None:
                state["pv1_v"] = round(pv1, 1)
        if len(vals) >= 3:
            pv2 = SolarParser._to_float(vals[2])
            if pv2 is not None:
                state["pv2_v"] = round(pv2, 1)

        # m50D: temperatures
        vals = parsed.get("m50D", ("", []))[1]
        if len(vals) >= 1:
            t = SolarParser._to_float(vals[0])
            if t is not None:
                state["pv_temp"] = round(t, 1)
        if len(vals) >= 2:
            t = SolarParser._to_float(vals[1])
            if t is not None:
                state["inverter_temperature_c"] = round(t, 1)
                state["bat_temp"] = round(t, 1)
        if len(vals) >= 3:
            t = SolarParser._to_float(vals[2])
            if t is not None:
                state["boost_temperature_c"] = round(t, 1)

        # wIkm: working mode (L=UTI, B=Battery, F=Fault, S=SUB)
        vals = parsed.get("wIkm", ("", []))[1]
        if len(vals) >= 1:
            code = vals[0].strip().upper()
            mode_map = {"L": "UTI", "B": "Battery", "F": "Fault", "S": "SUB"}
            mode = mode_map.get(code, code)
            state["working_mode"] = mode
            state["mode"] = mode
            if mode == "UTI":
                ...
                state.setdefault("mains_current_flow_direction", "Idle")

        # dnwz: battery type
        vals = parsed.get("dnwz", ("", []))[1]
        if len(vals) >= 1:
            bt = vals[0].strip()
            if bt:
                state["battery_type"] = bt

        # 6yCP: firmware version and build date
        vals = parsed.get("6yCP", ("", []))[1]
        if len(vals) >= 1:
            fw = vals[0].strip()
            if fw:
        ...
        # 6cuV: energy counters — unit is 0.1 kWh per count, sentinel = 32767 means no data
        vals = parsed.get("6cuV", ("", []))[1]
        if vals:
            def _energy(tok: str) -> Optional[float]:
                v = SolarParser._to_int(tok)
            ...
            _energy_map = [
                (0,  "pv_today_kwh"),
                (1,  "grid_import_today_kwh"),
                (2,  "pv_total_kwh"),
                (3,  "grid_import_total_kwh"),
                (4,  "load_today_kwh"),
                (5,  "load_total_kwh"),
                (11, "bat_charge_today_kwh"),
            ]
            for idx, key in _energy_map:
                if len(vals) > idx:
                    v = _energy(vals[idx])
                    if v is not None:
                        state[key] = v

        # kbr1: output priority, faults, voltage limits, mains power, efficiency
        vals = parsed.get("kbr1", ("", []))[1]
        if len(vals) >= 1:
            op = SolarParser._to_int(vals[0])
            if op is not None:
                state["output_priority"] = op
        if len(vals) >= 5:
            fc = SolarParser._to_int(vals[4])
            if fc is not None:
                state["active_fault_count"] = fc
        if len(vals) >= 6:
            v = SolarParser._to_float(vals[5])
            if v is not None:
                state["ac_ovp_v"] = round(v, 1)
        if len(vals) >= 8:
            v = SolarParser._to_float(vals[7])
            if v is not None:
                state["ac_uvp_v"] = round(v, 1)
        if len(vals) >= 10:
            mw = SolarParser._to_float(vals[9])
            if mw is not None:
                state["mains_power_w"] = round(mw, 1)
        if len(vals) >= 18:
            eff = SolarParser._to_float(vals[17])
            if eff is not None:
        ...
        if isinstance(mains_w, (int, float)) and mode_now == "UTI":
            ...
            state.setdefault("mains_wdrr_value", 0.0)

        # 6NWg: battery hard overvoltage protection voltage
        vals = parsed.get("6NWg", ("", []))[1]
        if len(vals) >= 1:
            v = SolarParser._to_float(vals[0])
            if v is not None:
                state["bat_hard_ovp_v"] = round(v, 1)

        # zwWS: fan 1 speed
        vals = parsed.get("zwWS", ("", []))[1]
        if len(vals) >= 1:
            spd = SolarParser._to_int(vals[0])
            if spd is not None:
                state["fan_1_speed"] = spd
                state["fan_1_status"] = "Running" if spd > 0 else "Stopped"

        # NRwZ / Yzqw: fan 2 speed (two block names observed, use whichever appears first)
        for _fan2_blk in ("NRwZ", "Yzqw"):
            vals = parsed.get(_fan2_blk, ("", []))[1]
            if len(vals) >= 1:
                spd = SolarParser._to_int(vals[0])
                if spd is not None:
                    state["fan_2_speed"] = spd
                    state["fan_2_status"] = "Running" if spd > 0 else "Stopped"
                    break

        # EJaH: lifetime event count
        vals = parsed.get("EJaH", ("", []))[1]
        if len(vals) >= 1:
            cnt = SolarParser._to_int(vals[0])
            if cnt is not None:
                state["lifetime_event_count"] = cnt
